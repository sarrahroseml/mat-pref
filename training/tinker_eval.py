...
def evaluate_model(
    ...
    split_name: str,
    out_path: Path,
    model_tag: str = "qwen3-8b-base",
    max_tokens: int = 1024,
) -> list[dict]:
    """Run evaluation using Tinker sampling client, with incremental saves."""
    ...
    log.info("Evaluating %d remaining (%d done)", len(remaining), total_done)
    ...
    for i, q in enumerate(remaining):
        ...
        log.info("[%d/%d] q=%s...", total_done + i + 1, len(questions), qid[:8])
        ...
        total_so_far = total_done + i + 1
        if total_so_far % 10 == 0 or total_so_far == len(questions):
            acc = correct / total_so_far * 100
            log.info("[%s] %d/%d  acc=%.1f%%", split_name, total_so_far, len(questions), acc)
...
def main():
    ...
    if args.base:
        ...
        model_tag = f"qwen3-8b-{Path(args.checkpoint).name}"
